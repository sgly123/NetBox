...
private:
    ...
    /**
     * @brief 🧠 智能提取纯净的RESP响应（保留用于兼容）
     * 自动检测并过滤NetBox协议头，只保留标准RESP数据
     */
    std::string extractPureRespResponse(const char* data, ssize_t length) {
        ...
        for (ssize_t i = 0; i < length; ++i) {
            char c = data[i];
            if (c == '+' || c == '-' || c == ':' || c == '$' || c == '*') {
                std::string respData(data + i, length - i);
                if (m_debugMode) {
                    std::cout << "[DEBUG] 提取RESP: " << respData.substr(0, 30) << "..." << std::endl;
                }
                return respData;
            }
        }
        ...
        if (m_debugMode) {
            std::cout << "[DEBUG] 未找到RESP数据，可能全是协议头" << std::endl;
        }
        return "";
    }
