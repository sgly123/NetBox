...
public:
    void onConnect(std::shared_ptr<Application::Context> ctx) override {
        ...
        ctx->send("欢迎连接到 TestProject 服务器!\n");
...
public:
    SimpleApplication() : Application("TestProject") {
        m_handler = std::make_shared<SimpleHandler>();
    }
    
    bool initialize() override {
        std::cout << "🔧 初始化 TestProject 服务器..." << std::endl;
    ...
    bool start() override {
        std::cout << "🚀 启动 TestProject 服务器..." << std::endl;
        // 这里会连接到NetBox的网络层
        return true;
    }
    
    void stop() override {
        std::cout << "🛑 停止 TestProject 服务器..." << std::endl;
    }
    
    void cleanup() override {
        std::cout << "🧹 清理 TestProject 服务器..." << std::endl;
    }
...
int main() {
    std::cout << "🌟 启动 TestProject 基础示例" << std::endl;
    ...
    std::cout << "👋 TestProject 已安全关闭" << std::endl;
    return 0;
}